...
// advertimos
$mensaje = "<h2 align=center><u>ACTUALIZAR DATOS DEL REGISTRO</u></h2> <br> ID_CUENTA: <b>$row[idcuenta]</b> // TITULAR:  <b>$row[titular]</b></center>";
}
// comprobamos si
// ha sido enviado el formulario
if(isset($_POST['actualizar']) && $_POST['actualizar'] == 'Actualizar'){
// comprobamos que no lleguen campos vacios
if(!empty($_POST['idcuenta']) && !empty($_POST['direccioncorreo']) && !empty($_POST['cuenta']) && !empty($_POST['password'])){
...
// la consulta UPDATE
$sqlUpdate = mysql_query("UPDATE emailscorp
...
<fieldset><center>
<b>Fecha inserción:</b> <input type="text" name="fechacreacion" value="<?php echo $row['fechacreacion']; ?>" />
<b>Descripción Actividad</b>  <input type="text" name="direccioncorreo" value="<?php echo $row['direccioncorreo']; ?>" /><br><br>
<b>Id_cuenta:</b> <input type="text" name="cuenta" value="<?php echo $row['cuenta']; ?>" />
...
<b>Cargo:</b> <input type="text" name="cargo" value="<?php echo $row['cargo']; ?>" /><br><br>
<b>Observaciones:</b> <input type="text" name="observaciones" value="<?php echo $row['observaciones']; ?>" />
<input type="hidden" name="idcuenta" value="<?php echo $row['idcuenta']; ?>" />
</center>
</fieldset><br>
<fieldset>
